...
import { CalendarIcon, Plus, Trash2, Pencil, X, Check, Zap, DollarSign } from "lucide-react";
...
import {
  ...
  useCreateLancamentoDisparo,
  useUpdateLancamentoDisparo,
  useDeleteLancamentoDisparo,
  useVendasRegistro,
...
export function LancamentoDisparoPage() {
  const [editingId, setEditingId] = useState<string | null>(null);

  const { data: lancamentos = [], isLoading } = useLancamentosDisparo();
  const { data: vendas = [] } = useVendasRegistro("disparo");
  const { data: closers = [] } = useClosers();

  const createLancamento = useCreateLancamentoDisparo();
  const updateLancamento = useUpdateLancamentoDisparo();
  const deleteLancamento = useDeleteLancamentoDisparo();
          ...
          <Card className="bg-card border-border">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-foreground">
                ...
                {editingId ? "Editar Lançamento" : "Novo Lançamento - Disparo"}
              </CardTitle>
            </CardHeader>
            <CardContent>
